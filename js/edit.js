...
let index = localStorage.getItem("editID");
nameInput.value = movies[index].name
yearInput.value = movies[index].year
descriptionInput.value = movies[index].description
imageInput.value = movies[index].image
ratingInput.value = (movies[index].rating.slice(0, -3))
dateInput.value = movies[index].releaseDate
directorInput.value = movies[index].director
summaryInput.value = movies[index].summary
genresArr = movies[index].genres
countryArr = movies[index].countryOfOrigin
languageArr = movies[index].language
starArr = movies[index].stars
...
addBtns.forEach(function (item) {
    ...
    // item["array"].forEach(function (e) {
    //     console.log(e)
    //    item["button"].nextElementSibling.textContent += e
    // })
    reloadSpan(item);
